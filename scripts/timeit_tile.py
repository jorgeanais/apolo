...
# ----------------------------------------------------
out_dir = path.join(dirconfig.test_tiling, '1024')
make_dir(out_dir)
catalog_filename = '/home/jorge/Documents/DATA/test/neighbors/bigtile_with_tiling_v3.fits'  # 1024
table = read_fits_table(catalog_filename)
tile_selection = table[table['tile'] == 100]
add_pseudocolor(tile_selection, color_excess=1.8)
...
delta_time = end - start
print(f'time for 1/1024: {delta_time}')
scores.meta.update({'TIME': delta_time})

score_filepath = path.join(out_dir, 'scores_1024_grid50_memory' + '.ecsv')
scores.write(score_filepath, format='ascii.ecsv')


# ----------------------------------------------------
out_dir = path.join(dirconfig.test_tiling, '512')
make_dir(out_dir)
catalog_filename = '/home/jorge/Documents/DATA/test/neighbors/bigtile_with_tiling_v4.fits'  # 512
table = read_fits_table(catalog_filename)
tile_selection = table[table['tile'] == 100]
add_pseudocolor(tile_selection, color_excess=1.8)
...
delta_time = end - start
print(f'time for 1/512: {delta_time}')
scores.meta.update({'TIME': delta_time})

score_filepath = path.join(out_dir, 'scores_512_grid50' + '.ecsv')
scores.write(score_filepath, format='ascii.ecsv')
